...
`research-cli help keys` — env file (`$HOME/.config/research-cli/env`, Windows `%APPDATA%\research-cli\env`), provider key table, missing-key behavior. Missing required key: ask the user, write the file, retry. Skip `x` if cookies are unset; skip `telegram search` if `TGSTAT_IDR`/`TGSTAT_SIRK` are unset; skip `telegram get`/`download`/`history` if `TELEGRAM_SESSION` is unset; do not block other providers. Errors name **tgstat** (cookies) vs **telegram** (user session).
...
All Telegram commands are `telegram …`. Credentials and errors still name the backend: **tgstat** (tgstat.com cookies for the public post index) vs **telegram** (user MTProto session for files/history). Do **not** join chats. Do not buy the TGStat Search API.

**Index** (`telegram search` / sources / mentions / export) needs `TGSTAT_IDR` (`tgstat_idrk`) and `TGSTAT_SIRK` (`tgstat_sirk`) from a logged-in tgstat.com browser — HttpOnly; copy like X cookies. Optional `TGSTAT_CSRK` / `TGSTAT_SETTINGS`. CSRF is fetched from `GET /search` each search. Dead cookies exit 2 and say **tgstat**. Skip these ops if unset. Website UI defaults to 7 days; the CLI omits dates unless `--start`/`--end` so the archive is used. Hard cap **1000 posts per query** (20/page).

Shared index filters: `--peer-type` `all|channel|chat` (default `all`). `--start` / `--end` `YYYY-MM-DD` or `DD.MM.YYYY`. `--sort` `date|views` (default `date`). `--country` `--language` `--category` slugs (`telegram catalogs`). `--minus-words`. `--views-range` `all|lt1000|1k-10k|10k` (default `all`). `--channel-id` numeric id from `sources`. `--source-sort` `members|freq` (default `members`). `--forwards` `all|hide|only` (default `all`; `--hide-forwards` is `hide`). `--hide-deleted` `--strong` `--extended` `--only-mentioned`.

**Session** (`login` / history / get / download) is Telethon user MTProto. **Not** Bot API. `login` is **once**. It writes `TELEGRAM_API_ID` / `TELEGRAM_API_HASH` / `TELEGRAM_SESSION` to `~/.config/research-cli/env` and `telegram.session` next to that file. Later file commands reuse that session — no phone, no code — until you revoke the device. Dead session exit 2 and say **telegram**. Skip these ops if unset. Optional `TELEGRAM_SESSION_FILE`.

Agent loop: `telegram search` (prefer `--peer-type channel`) → keep hits whose `telegram.has_media` is true and `telegram.private` is false → `telegram download` **only** `telegram.target`. Text posts: `telegram get`. Private joinchat hits are omitted unless `--private`. Link-preview `web_page` is not a file.

- `search` — public post index. `--limit` default 20 (cap 1000). `--offset`. `--download DIR` inspects via the Telegram session and saves real files. `--media` `document|photo|video`. `--allow-large` keeps files over 25MB. `--jobs` default 4 (concurrent ops on **one** Telegram connection; do not open extra sessions). `--private` includes joinchat hits. Each hit has `telegram.target` (the only download argument), `telegram.has_media`, `telegram.private`. JSON `provider` is `tgstat` (the index backend).
...
- `me` — Telegram user if `TELEGRAM_SESSION` is set; tgstat.com cookie probe if `TGSTAT_*` cookies are set.
- `discover` — public users/groups/channels by name (no join). `--limit` default 20.
- `history` — messages from a public `@username` / `t.me/user`. `--search` is in-chat search. `--limit` default 50. `--offset-id` / `--min-id`. Invite links are not history; use `resolve`.
- `resolve` — `@username` / `t.me/user`, or peek `t.me/+invite` / `t.me/joinchat/…` without joining. Invite JSON `status` is `already_member`, `request_needed`, `preview`, `peek`, or `expired`.
- `get` — one message body (text + Telegram media classification). Use this for text posts (`has_media` false).
- `download` — one message **file** (`document|photo|video`). Link-preview `web_page` is `no media`. Same `telegram.target` as search hits. Public `@channel`: no join. Private: already a member only.
